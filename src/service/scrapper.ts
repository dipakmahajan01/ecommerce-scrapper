...
function extractBaseModel(title: string | null): string {
  ...
  let baseModel = title
    ...
    .replace(/\b(?:white|black|blue|red|green|yellow|pink|purple|orange|silver|gold|titanium|graphite|space|gray|grey)\b/gi, "")
...
async function scrapePage(page: any): Promise<Product[]> {
  ...
  try {
    ...
    console.log(`[Flipkart Scraper] Product cards selector not found, checking page content...`);
  ...
  const productCards = await page.$$eval(
    ...
    (cards: Element[]) => {
      return cards.map((card: Element) => {
        ...
        const link = relativeHref
          ? (relativeHref.startsWith("http") ? relativeHref : "https://www.flipkart.com" + relativeHref)
          : null;

        // Image - using the actual class name from HTML
        const imgElem = card.querySelector("img.UCc1lI") as HTMLImageElement;
        const image = imgElem?.getAttribute("src") || imgElem?.getAttribute("data-src") || null;
    ...
    console.log(`[Flipkart Scraper] Error extracting products: ${err.message}`);
    return [];
  });

  console.log(`[Flipkart Scraper] Found ${productCards.length} product cards on this page`);
...
export async function scrapeFlipkartSearch(url: string, retryCount = 0, maxPages = 4): Promise<Product[]> {
  ...
  try {
    console.log(`[Flipkart Scraper] Starting fresh browser instance for: ${url}`);
    ...
    try {
      ...
      console.log(`[Flipkart Scraper] Could not clear cookies/cache before navigation: ${e}`);
    }
    ...
    console.log(`[Flipkart Scraper] Page 1: Found ${page1Results.length} products`);

    // Scrape additional pages (2, 3, 4, etc.)
    for (let pageNum = 2; pageNum <= maxPages; pageNum++) {
      try {
        console.log(`[Flipkart Scraper] Attempting to navigate to page ${pageNum}...`);
        ...
        try {
          ...
          console.log(`[Flipkart Scraper] Pagination not found, stopping at page ${pageNum - 1}`);
          break;
        }
        ...
        if (!pageButton) {
          console.log(`[Flipkart Scraper] Page ${pageNum} button not found, stopping pagination`);
          break;
        }
        ...
        try {
          ...
          console.log(`[Flipkart Scraper] Products not loaded on page ${pageNum}, skipping...`);
          continue;
        }
        ...
        console.log(`[Flipkart Scraper] Page ${pageNum}: Found ${pageResults.length} products`);

        // Small delay between pages
        await new Promise(resolve => setTimeout(resolve, 1000));
        
      } catch (error: any) {
        console.error(`[Flipkart Scraper] Error scraping page ${pageNum}: ${error.message}`);
    ...
    try {
      ...
      if (currentUrl !== url && !currentUrl.includes('search')) {
        console.log(`[Flipkart Scraper] Possible redirect detected: ${currentUrl}`);
      }
    }
    ...
    console.log(`[Flipkart Scraper] Found ${productCards.length} product cards`);
    ...
    if (results.length === 0) {
      console.log(`[Flipkart Scraper] No products found, trying alternative selectors...`);
      
      // Check if page has any content at all
      const bodyText = await page.$eval('body', (el: any) => el.textContent || '');
      const hasContent = bodyText && bodyText.length > 100;
      console.log(`[Flipkart Scraper] Page has content: ${hasContent}, body length: ${bodyText?.length || 0}`);
      
      // Try to find product cards using a different approach
      const cardCount = await page.$$eval("div[data-id]", (cards: any[]) => cards.length);
      console.log(`[Flipkart Scraper] Found ${cardCount} div[data-id] elements`);
      ...
      if (cardCount > 0) {
        const altProducts = await page.evaluate(() => {
          ...
          cards.forEach((card, index) => {
            ...
            const linkUrl = href ? (href.startsWith("http") ? href : "https://www.flipkart.com" + href) : null;
        ...
        console.log(`[Flipkart Scraper] Alternative method found ${altProducts.length} products`);
      } else {
        console.log(`[Flipkart Scraper] No product cards found. Page might be blocked or structure changed.`);
      }
    }
    ...
    try {
      ...
      if (browser) {
        await browser.close();
        console.log(`[Flipkart Scraper] Browser instance closed and cleaned up`);
      }
    } catch (e) {
  ...
  if (results.length === 0 && retryCount < MAX_RETRIES) {
    console.log(`[Flipkart Scraper] No results found, retrying... (attempt ${retryCount + 1}/${MAX_RETRIES})`);
  ...
  console.log(`[Flipkart Scraper] Total products found: ${results.length}, Unique models (by color+RAM): ${uniqueResults.length}`);
