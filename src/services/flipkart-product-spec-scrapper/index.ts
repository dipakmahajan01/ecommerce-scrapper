...
const product = async (link, type) => {
  ...
  try {
    ...
    try {
      var webPageContents = await (
        await fetch("https://www.flipkart.com/" + uri)
      ).text();
      ...
      var inStock =
        !doesExist(
          webPageContents.split("This item is currently out of stock</div>")
        ) || comingSoon;
      ...
      return {
        ...
        possible_solution:
          "Validate your link and try removing https://www.flipkart.com from your product link",
    ...
    if (webPageContents.includes("<h1")) {
      productName = webPageContents
        ...
        .replace(/<!-- --/g, "");
      try {
        var subName = webPageContents
          .split('class="B_NuCI')[1]
          .split("</span>")[0]
          .split(">")[1]
          .replace(/<!-- -->/g, "")
          .replace(/<!-- --/g, "");
    ...
    if (thumbnails.length == 0) {
      try {
        ...
        if (thumb.length == 1) {
          thumb = webPageContents.split(
            `alt="${thumbnailDetailsUsingName.slice(0, 5)}}`
          );
        }
    ...
    try {
      ...
      for (let i of imgArray) {
        ...
        if (
          !i.includes("promos") &&
          i.slice(-2).match(/^\d+$/) &&
          parseInt(i.slice(-2)) >= 50
        ) {
          if (!thumbnails.includes(i)) thumbnails.push(i);
        }
      }
    } catch (e) {}
    ...
    let fassured = false;
    try {
      var fAssuredDetails = webPageContents
        .split("<h1")[1]
        .split("Product Description")[0]
        .split("fk-cp-zion/img/fa_62673a.png");
    ...
    try {
      ...
      if (doesExist(smallUri)) {
        smallUri =
          lastEntry(smallUri[smallUri.length - 2].split('"')) +
          "product.share.pp";
        properURI = smallUri;
      }
      if (properURI[0] == "/") {
        properURI = "https://www.flipkart.com" + properURI;
      }
      let x = 0;
      if (uri.split("/")[0] == "") {
        x = 1;
      ...
      if (
        properURI2.length < properURI.length ||
        doesExist(String(properURI).toLowerCase().split("login"))
      ) {
    ...
    try {
      let highlightsDetails = webPageContents
        .split("Highlights")[1]
        .split("</ul>")[0]
        .replace(/<\/li>/g, "")
        .split("<li");
    ...
    // product variants
    const $ = cheerio.load(webPageContents);

    // Helper to iteratively extract variant info (e.g. ram/color) by swatch number
    function extractSwatchVariants(type, extractValueFn) {
      const arr = [];
      let idx = 0;
      while (true) {
        const selector = `#swatch-${idx}-${type}`;
        if (!$(selector).length) break;

        const val = extractValueFn($(selector), idx);
        // If no valid value, stop iterating
        if (!val) break;
    ...
    // Ram - extract a-tag text within swatch
    const ramVariants = extractSwatchVariants("ram", ($swatch) => {
      const a = $swatch.find("a");
      if (!a.length) return null;
      return a.text().trim();
    });

    // Color - extract a-tag data-img and nested ._3gBjOM
    const colorVariants = extractSwatchVariants("color", ($swatch) => {
      const a = $swatch.find("a");
      if (!a.length) return null;
      const dataImg = a.attr("data-img");

      // Find nested ._3gBjOM (any depth within $swatch)
      const colorNameEl = $swatch.find("._3gBjOM").first();
      const colorName = colorNameEl.length ? colorNameEl.text().trim() : null;

      return {
        colorImg: dataImg || null,
        colorName: colorName || null,
      };
    });

    // Storage - also follow same swatch index pattern, extract <a> text for each storage choice
    const storageVariants = extractSwatchVariants("storage", ($swatch) => {
      const a = $swatch.find("a");
      if (!a.length) return null;
      return a.text().trim();
    });
    ...
    if (!minimumResult) {
      ...
      if (inStock)
        try {
          ...
          let offer_section = webPageContents.split(
          ...
          for (var i = 0; i < offer_section.length; i++) {
            try {
              offer_section[i] = offer_section[i]
                .split("<li")[1]
                .split("<div")[0]
                .replace(/<span>/g, " : ")
                .replace(/<\/span>/g, "")
      ...
      var specs = [];
      try {
        let specsDetails = webPageContents
          .split("Specifications</div>")[1]
          .split(">Safe and Secure Payments.")[0]
          .replace(/&amp;/g, "&")
          .split("</div><table");
    ...
    if (inStock)
      try {
        ...
        try {
          var seller_rating = lastEntry(seller.split(star)[0].split(">")).split(
            "<"
          )[0];
          if (seller_rating.length <= 3)
            resultJson["seller"]["seller_rating"] = parseFloat(seller_rating);
        } catch (e) {}
...
function makeThumbnails(locationArray) {
  ...
  for (var i = 1; i < locationArray.length; i++) {
    try {
      var thumbnailDetails = locationArray[i]
        .split("</div>")[0]
